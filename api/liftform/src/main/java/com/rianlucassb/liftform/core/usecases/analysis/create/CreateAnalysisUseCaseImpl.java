...
public class CreateAnalysisUseCaseImpl implements CreateAnalysisUseCase {
    ...
    public CreateAnalysisUseCaseImpl(VideoAnalysisRepository videoAnalysisRepository, VideoStorage videoStorage, UserRepository userRepository) {
    ...
    public CreateAnalysisUseCaseOutput execute(CreateAnalysisUseCaseInput input) {
        ...
        userRepository.findById(userId).orElseThrow(() -> new UserNotFoundAuthException("User with received ID not found"));

        String key = String.format("videos/%s/%s/%s.mp4", input.exerciseType(), input.userId(), UUID.randomUUID());

        VideoAnalysis videoAnalysis = VideoAnalysis.initialize(
                userId,
                input.exerciseType(),
                key
        );
